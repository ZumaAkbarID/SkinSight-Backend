...
export async function sendOtp(
  ...
  const { otp, expiresAt } = generateOtp(purpose === 'register' ? 5 : 4)
  ...
  await mail.use('brevo').sendLater((message) => {
    message
      .subject(
        `OTP Verification - SkinSight (${purpose === 'register' ? 'Registration' : 'Forgot Password'})`
      )
